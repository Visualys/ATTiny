...
void serial_send(uint8_t TX, char* text, uint32_t TXbauds) {                   // TX must be on PORTA
    ...
    TCCR0B = 0;                                                                // stop timer
    TCCR0A = 0b00000010;                                                       // timer0 CTC Mode
    TCNT0 = 0;                                                                 // set counter to zero
    PORTA |= pinTX;                                                            // set pin HIGH
    if(TXbauds==9600){ OCR0A = 107;}
    else if(TXbauds==115200){ OCR0A = 8;}
    TCCR0B = 1;                                                                // start timer with 1/1 speed
...
uint8_t serial_read(uint8_t RX, char* msg, uint32_t RXbauds) {
    ...
    TCCR0A = 0b00000010;                                                       // timer0 CTC Mode
    TCNT0 = 0;                                                                 // set counter to zero
    if(RXbauds==9600){ OCR0A = 107;}
    else if(RXbauds==115200){ OCR0A = 8;}
    while ( b != 10 ) {  
        while(!(PINA & pinRX) );
        TCCR0B = 0;                                                            // stop timer
        TIFR0 = 0b00000111;                                                    // reset overflow flag
